...
class TestRealPackages(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            subprocess.check_call([shutil.which("npm"), "--version"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    @unittest.skipIf(not os.environ.get("RUN_REAL_PACKAGE_TESTS"), "Skipping real package tests. Set RUN_REAL_PACKAGE_TESTS=1 to run")
